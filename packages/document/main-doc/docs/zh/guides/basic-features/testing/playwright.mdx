...
```ts title="routes/about/page.tsx"
import { Link } from '@modern-js/runtime/router';
...
Playwright 将模拟用户使用三种浏览器（Chromium、Firefox 和 Webkit）导航你的应用程序，这需要你的 Modern.js 服务器保持运行。我们推荐在生产环境产物下运行你的测试用例，你可以执行 `pnpm run build` 和 `pnpm run serve` 在本地模拟生产环境。
